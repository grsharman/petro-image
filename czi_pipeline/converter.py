#!/usr/bin/env python3
"""Standalone AxioScan 7 CZI metadata and bounded-memory DZI converter."""
...
PIXEL_ASPECT_RELATIVE_TOLERANCE = 1e-6
...
def get_pixel_sizes_microns(metadata: dict) -> dict[str, float]:
    """Return calibrated pixel extents keyed by axis (X, Y, or Z)."""
    distances = as_list(metadata.get("Scaling", {}).get("Items", {}).get("Distance"))
    sizes = {}
    for distance in distances:
        axis = str(distance.get("@Id", "")).upper()
        if axis in {"X", "Y", "Z"} and distance.get("Value") is not None:
            sizes[axis] = float(distance["Value"]) * 1_000_000
    return sizes


def pixel_calibration_status(metadata: dict) -> tuple[float | None, float | None, bool | None]:
    sizes = get_pixel_sizes_microns(metadata)
    x = sizes.get("X")
    y = sizes.get("Y")
    square = None if x is None or y is None else math.isclose(
        x,
        y,
        rel_tol=PIXEL_ASPECT_RELATIVE_TOLERANCE,
        abs_tol=0.0,
    )
    return x, y, square
...
def inspect_czi(source: Path) -> dict:
    with czi.open_czi(str(source)) as reader:
        ...
        microns_per_pixel_x, microns_per_pixel_y, has_square_pixels = (
            pixel_calibration_status(metadata)
        )
        ...
        return {
            ...
            "micronsPerPixel": microns_per_pixel_x,
            "pixelSizeMicrometers": {
                "x": microns_per_pixel_x,
                "y": microns_per_pixel_y,
            },
            "hasSquarePixels": has_square_pixels,
            "pixelsPerMeter": (
                1_000_000 / microns_per_pixel_x if microns_per_pixel_x else None
            ),
            "channels": details,
...
def main() -> int:
    ...
    with czi.open_czi(str(source)) as reader:
        ...
        microns_per_pixel, microns_per_pixel_y, has_square_pixels = (
            pixel_calibration_status(metadata)
        )
        if has_square_pixels is False:
            raise SystemExit(
                "CZI has non-square calibrated pixels: "
                f"X={microns_per_pixel:g} um, Y={microns_per_pixel_y:g} um"
            )
        pixels_per_meter = None
        if microns_per_pixel:
            physical_width_meters = roi[2] * microns_per_pixel / 1_000_000
            pixels_per_meter = output_width / physical_width_meters
